...
function configure_lr-flycast() {
    ...
    # On RPI 3 and similar, disable some video effects to increase performance
    if isPlatform "armv7" || isPlatform "rpi3"; then
        setRetroArchCoreOption "reicast_anisotropic_filtering"  "off"
        setRetroArchCoreOption "reicast_fog"                    "disabled"
        setRetroArchCoreOption "reicast_mipmapping"             "disabled"
        setRetroArchCoreOption "reicast_volume_modifier_enable" "disabled"
        setRetroArchCoreOption "reicast_auto_skip_frame"        "more"
    fi
