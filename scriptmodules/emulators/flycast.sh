...
function configure_flycast() {
    local emu_params=("--config window:height=%YRES%" "--config window:width=%XRES%")
    local sys
    ...
    # add a minimal conf on 1st installation
    if [[ ! -f "$md_conf_root/dreamcast/emu.cfg" ]]; then
        local temp_conf="$(mktemp)"
        _generate_conf_flycast "$temp_conf"
        copyDefaultConfig "$temp_conf" "$md_conf_root/dreamcast/emu.cfg"
    fi
    # on 1st install create a new SDL keyboard mapping
...
function _generate_conf_flycast() {
    ...
    local have_effects="yes" # whether to disable the effect or not
...
rend.useMipMaps = $have_effects
rend.Fog = $have_effects
rend.ModifierVolumes = $have_effects
